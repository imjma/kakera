...
| RedNote | `http(s)://xhslink.com/m/ID` or `/o/ID`, or the resolved `xiaohongshu.com/explore/ID` or `/item/ID` share URL with its `xsec_token` | Reads public page data and images; no login-only posts. Capture does not download video; Transient Telegram Delivery sends covers then the MP4. |
